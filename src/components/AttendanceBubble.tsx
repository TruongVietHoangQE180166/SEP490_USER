...
export const AttendanceBubble = () => {
  ...
  const thisMonthCount = Array.from(attendedDays).filter(
    (d) => new Date(d).getMonth() === month && new Date(d).getFullYear() === year
  ).length;

  if (!isMounted || !isAuthenticated || isAuthPage) return null;
  ...
  return (
    <>
      ...
      >
        ...
        {/* Badge: chưa điểm danh hôm nay */}
        {!todayAttended && !isOpen && (
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}

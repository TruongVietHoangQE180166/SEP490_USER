...
interface CourseCardProps {
  ...
  onToggleStatus: (id: string, currentStatus: string, title: string) => void;
}
  ...
  return (
    <div className="group relative flex flex-col bg-card rounded-xl border border-border/40 overflow-hidden shadow-sm hover:shadow-lg hover:shadow-primary/8 transition-all duration-300 hover:-translate-y-0.5">
      ...
      <div className="border-t border-border/40 grid grid-cols-2 divide-x divide-border/40">
        ...
        >
          <Eye size={15} />
          <span className="text-[9px] font-bold uppercase tracking-wide">Xem chi tiết</span>
        </button>

        {/* Toggle status: Duyệt / Từ chối */}
        <button
          onClick={() => onToggleStatus(course.id, course.status, course.title)}
          title={isPublished ? 'Từ chối khoá học' : 'Duyệt khoá học'}
          className={`flex flex-col items-center justify-center py-3 gap-1 transition-all duration-200 cursor-pointer ${
            isPublished
        ...
        >
          {isPublished ? (
            <>
              <XCircle size={15} />
              <span className="text-[9px] font-bold uppercase tracking-wide">Từ chối</span>
            </>
          ) : (
            <>
              <CheckCircle2 size={15} />
              <span className="text-[9px] font-bold uppercase tracking-wide">Duyệt</span>
            </>
          )}
        </button>
      </div>

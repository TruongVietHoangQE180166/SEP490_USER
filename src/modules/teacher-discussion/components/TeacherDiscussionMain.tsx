...
import { teacherDiscussionState$, teacherDiscussionActions } from '../store';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { CourseDiscussion } from '@/modules/course/components/CourseDiscussion';

export const TeacherDiscussionMain = observer(() => {
  const state = teacherDiscussionState$.get();
  const { courses, selectedCourseId, isLoading, error } = state;
  
  useEffect(() => {
    // Reset selection when entering AND leaving the page
    teacherDiscussionState$.selectedCourseId.set(null);
    return () => {
      teacherDiscussionState$.selectedCourseId.set(null);
    };
  }, []);
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-1">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center p-12 gap-3 text-muted-foreground/50">
              ...
              <p className="text-[11px] font-black uppercase tracking-widest">Đang tải danh sách...</p>
            </div>
          ) : courses.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-12 text-center gap-4">
              <Layout size={48} className="text-muted/10" />
              <p className="text-xs font-bold text-muted-foreground/40 leading-relaxed px-4">Bạn chưa có khóa học nào để quản lý thảo luận.</p>
            </div>
          ) : (
            courses.map(course => (
              <button
                key={course.id}
                onClick={() => teacherDiscussionActions.selectCourse(course.id)}
                className={cn(
      ...
      <div className={cn(
        ...
        {isLoading ? (
           <div className="flex flex-col items-center gap-5 text-center animate-in fade-in zoom-in duration-500">
              ...
              <p className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground/40 animate-pulse">Đang chuẩn bị dữ liệu...</p>
           </div>
        ) : !selectedCourseId ? (
          <div className="text-center space-y-8 max-w-sm px-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="space-y-3">
              <h2 className="text-2xl font-black tracking-tighter">Quản lý Thảo luận</h2>
              <p className="text-muted-foreground/60 text-sm font-medium leading-relaxed">
                Chọn một khóa học từ danh sách bên trái để xem các thảo luận mới nhất từ học viên của bạn.
              </p>
            </div>
          </div>
        ) : (
          <>
            {/* Integrated Chat Header for Teacher Dashboard */}
            <div className="px-6 py-4 border-b border-border/40 flex items-center justify-between gap-6 bg-background/50 backdrop-blur-md sticky top-0 z-20 shrink-0">
               <div className="flex items-center gap-4 min-w-0 flex-1 overflow-hidden">
                  ...
                  <div className="min-w-0 flex-1 group/title">
                     <h3 className="text-[15px] font-black tracking-tight truncate w-full group-hover/title:text-primary transition-colors duration-300" title={selectedCourse?.title}>
                     ...
                     <div className="flex items-center gap-2 text-[10px] font-bold text-emerald-500/80">
                        ...
                        <span className="uppercase tracking-widest">Kênh thảo luận trực tuyến</span>
                     </div>
                  </div>
               </div>

...
export const OrderPanel = observer(function OrderPanel() {
  ...
  // Nếu là Mua: quantity state đại diện cho số tiền USDT
  // Nếu là Bán: quantity state đại diện cho số lượng XAUT
  const estCost = side === 'LONG' 
    ? qtyInput 
    : qtyInput * (kind === 'LIMIT' ? limitPrice : currentPrice);

  const priceForCalc = kind === 'LIMIT' ? (limitPrice || currentPrice) : currentPrice;
  const xautReceivedValue = side === 'LONG' ? (qtyInput / priceForCalc) : 0;

  // Tính phần trăm tài sản hiện tại
  const maxAvailable = side === 'LONG' ? balance : gold;
  const currentPercent = maxAvailable > 0 ? (qtyInput / maxAvailable) * 100 : 0;

  const handlePercentChange = (percent: number) => {
    const newVal = (maxAvailable * (percent / 100));
    // Format đẹp mắt
    setQuantity(side === 'LONG' ? newVal.toFixed(0) : newVal.toFixed(4));
  };

  return (
    <div className="flex flex-col gap-3 p-4 bg-card h-full overflow-y-auto no-scrollbar">
      ...
      <div className="flex flex-col gap-2">
        <NumericInput 
          id="tut-order-amount"
          label={side === 'LONG' ? "Số tiền (USDT)" : "Số lượng (XAUT)"} 
          value={quantity} 
          onChange={setQuantity} 
          step={side === 'LONG' ? 10 : 0.001}
        />
        ...
        <div id="tut-order-slider" className="px-1 pt-1 flex flex-col gap-2">
          ...
          <div className="flex justify-between text-[9px] font-black text-muted-foreground/50 uppercase tracking-tighter">
            ...
            <span onClick={() => handlePercentChange(100)} className="hover:text-primary cursor-pointer transition-colors">Max</span>
          </div>
        </div>
      </div>

      {/* ── TP / SL ── */}
      {side === 'SHORT' && (kind === 'TP/SL' || kind === 'LIMIT') && (
      ...
      {qtyInput > 0 && (
        <div className="flex flex-col gap-2 p-3 bg-primary/5 border border-primary/20 rounded-lg">
           <div className="flex items-center justify-between text-xs">
              <span className="text-muted-foreground">{side === 'LONG' ? 'Chi phí dự kiến' : 'Tổng giá trị nhận'}</span>
           ...
           {side === 'LONG' && (
             <div className="flex items-center justify-between text-xs pt-2 border-t border-primary/10">
                <span className="text-muted-foreground">XAUT nhận được</span>
      ...
      >
        {isLoading ? 'Đang xử lý...' : (kind === 'MARKET' ? 'Vào lệnh Thị trường' : kind === 'LIMIT' ? 'Đặt lệnh Giới hạn' : 'Đặt lệnh TP/SL')} —{' '}
